...
public class FormController {
    ...
    @ResponseBody
    public MyResponse addForm(@RequestParam("form_content") String form_content,@RequestParam("form_file") MultipartFile form_file, @RequestHeader("Authorization")String token) throws IOException {
        if (JwtUtil.verifyToken(token)){
            String file_path = formService.uploadFile(form_file, JwtUtil.getInfoFromToken(token));
            formService.addForm(new Form(form_content,file_path,JwtUtil.getInfoFromToken(token)));
